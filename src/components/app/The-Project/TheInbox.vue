      ...
      <div class="mt-5">
        <ul>
          <li
            ...
            :key="task.id"
            class="goal relative cursor-pointer flex gap-2 before:content-[''] before:absolute before:-left-7 before:w-[100px] before:h-full py-2"
          >
            <button
...
let taskList = ref([])
axios
  .get('http://localhost:3000/Backend/api/getAllTodo/')
  .then((response) => {
